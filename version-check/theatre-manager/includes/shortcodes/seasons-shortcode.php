...
function tm_shortcode_seasons($atts) {
	...
	$base_font = esc_attr(get_option("tm_season_base_font", "Arial, sans-serif"));
	$style = "background-color: $bg_color; color: $text_color; font-family: $base_font; border-width: {$border_width}px; border-style: solid; border-radius: {$rounded}px; box-shadow: $shadow;";
...
/**
 * Shortcode to display the season(s) with the "Is Current Season" flag set.
 * Usage: [TM_Current_Season]  or  [tm_current_season]
 */
function tm_shortcode_current_season($atts) {
    $atts = shortcode_atts(array(
        'exclude' => '',
    ), $atts, 'tm_current_season');
    ...
    if (empty($seasons)) {
        return '<div class="tm-current-season"><p><em>No current season is set.</em></p></div>';
    }
    ...
    ob_start();
    echo '<div class="tm-shortcode-wrapper tm-current-season-wrapper" style="' . $style . '">';

    foreach ($seasons as $season) {
        $id = $season->ID;
        echo '<div class="tm-entry tm-seasons-entry tm-season-current">';
        echo '<h3>' . esc_html($season->post_title) . ' <span class="tm-season-current-label">(' . esc_html__('Current Season', 'theatre-manager') . ')</span></h3>';
...
add_shortcode('TM_Current_Season', 'tm_shortcode_current_season');
add_shortcode('tm_current_season', 'tm_shortcode_current_season');
?>
