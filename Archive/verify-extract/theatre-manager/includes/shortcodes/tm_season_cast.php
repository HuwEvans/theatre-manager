...
function tm_season_cast_shortcode($atts) {
    ...
    $bg_color = function_exists('tm_sanitize_css_color') ? tm_sanitize_css_color(get_option('tm_show_bg_color', '#ffffff'), '#ffffff') : '#ffffff';
    ...
    $border_width = 0;
    $rounded = (bool) get_option('tm_show_rounded');
    $border_radius = $rounded ? absint(get_option('tm_show_radius', '20')) : 0;
    $shadow_enabled = (bool) get_option('tm_show_shadow');
    $base_font = get_option('tm_show_base_font', 'Arial, sans-serif');

    $style = 'background-color:' . esc_attr($bg_color) . '; color:' . esc_attr($text_color) . '; font-family:' . esc_attr($base_font) . '; border-width:' . esc_attr($border_width) . 'px; border-color:' . esc_attr($border_color) . '; border-radius:' . esc_attr($border_radius) . 'px; border-style:solid;';
    if ($shadow_enabled) {
                ...
                foreach ($cast_members as $cast) {
                    ...
                    if ($atts['cast_layout'] === 'list') {
                        ...
                        if ($atts['show_cast_images'] === 'true' && $picture) {
                            $picture_url = tm_get_image_url($picture);
                            if ($picture_url) {
							    $output .= '<img src="' . esc_url($picture_url) . '" alt="' . esc_attr($character) . '" class="tm-cast-image">';
                            }
                        }
